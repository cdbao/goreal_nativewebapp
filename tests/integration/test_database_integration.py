...
class TestPlayerModel:
    ...
    def test_create_player(self, test_session):
        ...
        retrieved_player = (
            test_session.query(Player).filter_by(player_id="TEST123").first()
        )
    ...
    def test_player_relationships(self, test_session):
        ...
        player_challenge = PlayerChallenge(
            player_id="TEST123", challenge_id="C01", status="completed"
        )
...
class TestChallengeModel:
    ...
    def test_create_challenge(self, test_session):
        ...
        retrieved_challenge = (
            test_session.query(Challenge).filter_by(challenge_id="C01").first()
        )
...
class TestPlayerChallengeModel:
    ...
    def test_create_player_challenge(self, test_session):
        ...
        retrieved_pc = (
            test_session.query(PlayerChallenge)
            .filter_by(player_id="TEST123", challenge_id="C01")
            .first()
        )
    ...
    def test_player_challenge_foreign_keys(self, test_session):
        ...
        pc_no_challenge = PlayerChallenge(
            player_id="TEST123", challenge_id="NONEXISTENT"
        )
...
class TestActivityLogModel:
    ...
    def test_create_activity_log(self, test_session):
        ...
        retrieved_log = (
            test_session.query(ActivityLog)
            .filter_by(player_id="TEST123", action="challenge_received")
            .first()
        )
    ...
    def test_activity_log_json_details(self, test_session):
        ...
        complex_details = {
            ...
            "metrics": {"time_spent": 300.5, "attempts": 3},
        }

        log_entry = ActivityLog(
            player_id="TEST123", action="challenge_completed", details=complex_details
        )
        ...
        retrieved_log = (
            test_session.query(ActivityLog)
            .filter_by(player_id="TEST123", action="challenge_completed")
            .first()
        )
...
class TestAchievementSystem:
    ...
    def test_create_achievement(self, test_session):
        ...
        retrieved_achievement = (
            test_session.query(Achievement).filter_by(achievement_id="ACH001").first()
        )
    ...
    def test_player_achievement_earning(self, test_session):
        ...
        retrieved_pa = (
            test_session.query(PlayerAchievement)
            .filter_by(player_id="TEST123", achievement_id="ACH001")
            .first()
        )
...
class TestDataIntegrity:
    ...
    def test_cascade_delete_player(self, test_session):
        ...
        player = Player(player_id="TEST123", player_name="TestPlayer")
        challenge = Challenge(
            challenge_id="C01", title="Test", description="Desc", reward_points=100
        )
        player_challenge = PlayerChallenge(player_id="TEST123", challenge_id="C01")
        achievement = Achievement(
            achievement_id="ACH001", title="Test Achievement", description="Desc"
        )
        player_achievement = PlayerAchievement(
            player_id="TEST123", achievement_id="ACH001"
        )
        ...
        test_session.add_all(
            [player, challenge, player_challenge, achievement, player_achievement]
        )
        test_session.commit()

        # Verify data exists
        assert (
            test_session.query(PlayerChallenge).filter_by(player_id="TEST123").count()
            == 1
        )
        assert (
            test_session.query(PlayerAchievement).filter_by(player_id="TEST123").count()
            == 1
        )
        ...
        assert (
            test_session.query(Achievement).filter_by(achievement_id="ACH001").count()
            == 1
        )
    ...
    def test_complex_query_player_stats(self, test_session):
        ...
        stats = (
            test_session.query(
                ...
                func.sum(
                    func.case([(PlayerChallenge.status == "completed", 1)], else_=0)
            ...
            .join(Challenge)
            .filter(PlayerChallenge.player_id == "TEST123")
            .first()
        )
