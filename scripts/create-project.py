...
class ProjectCreator:
    ...
    def validate_inputs(
        self, template_config: Dict[str, Any], inputs: Dict[str, Any]
    ) -> bool:
        ...
        for var_name, var_config in variables.items():
            ...
            if var_type == "integer":
                ...
                if value not in choices:
                    print(
                        f"❌ Variable '{var_name}' must be one of: {', '.join(choices)}"
                    )
                    return False
    ...
    def process_template_file(
        self, src_path: Path, dst_path: Path, variables: Dict[str, Any]
    ):
                ...
                if isinstance(content, dict):
                    ...
                    if template_file.exists():
                        self.process_template_file(
                            template_file, current_path, variables
                        )
                    else:
                        # Create empty file
                        current_path.touch()
    ...
    def generate_secrets(self, variables: Dict[str, Any]) -> Dict[str, str]:
        ...
        if variables.get("use_jwt_auth"):
            secrets_map["JWT_SECRET_KEY"] = "".join(
                secrets.choice(alphabet) for _ in range(32)
            )

        # Generate database password
        db_alphabet = string.ascii_letters + string.digits
        secrets_map["DB_PASSWORD"] = "".join(
            secrets.choice(db_alphabet) for _ in range(16)
        )

        if variables.get("use_redis"):
            secrets_map["REDIS_PASSWORD"] = "".join(
                secrets.choice(db_alphabet) for _ in range(16)
            )
    ...
    def create_project(
        ...
        if output_path.exists() and any(output_path.iterdir()):
            response = input(
                f"⚠️  Directory '{output_dir}' already exists and is not empty. Continue? (y/N): "
            )
        ...
        try:
            ...
            self.create_project_structure(
                template_dir, output_path, template_config, inputs
            )
            ...
            if shutil.which("git"):
                subprocess.run(
                    ["git", "init"], cwd=output_path, check=False, capture_output=True
                )
                subprocess.run(
                    ["git", "add", "."],
                    cwd=output_path,
        ...
        if not post_steps:
            ...
            if variables.get("use_docker"):
                ...
                if template_config.get("name") == "flask-api":
                    print(
                        f"   flask --app {variables['project_name'].replace('-', '_')}.api.app:create_app() run"
                    )
...
def main():
    ...
    if config_inputs and "project_name" in config_inputs:
        output_dir = (
            args.output if args.output != "." else config_inputs["project_name"]
        )
    else:
