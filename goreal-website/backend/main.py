...
if GEMINI_API_KEY:
    ...
    print(" Cảnh báo: GEMINI_API_KEY chưa được thiết lập. Chatbot sẽ dùng câu trả lời mẫu.")
...
def get_live_content():
    try:
        ...
        creds_path = os.path.join(os.path.dirname(__file__), 'goreal-470006-ac9c0ea86e0c.json')
        creds = ServiceAccountCredentials.from_json_keyfile_name(creds_path, scope)
        client = gspread.authorize(creds)
        sheet = client.open(WEBSITE_CONTENT_SHEET_NAME)
        
        hero_ws = sheet.worksheet("Hero"); hero_data = dict(hero_ws.get_all_values()[1:])
        about_ws = sheet.worksheet("AboutCards"); about_data = about_ws.get_all_records()
        how_ws = sheet.worksheet("HowToSteps"); how_data_raw = how_ws.get_all_records()[0]
        how_data = {"title": how_data_raw.get("title"), "steps": [how_data_raw.get(f"step{i+1}") for i in range(4)]}
        chatbot_ws = sheet.worksheet("Chatbot"); chatbot_data = dict(chatbot_ws.get_all_values()[1:])
        
        content = {"hero": hero_data, "about": about_data, "how_it_works": how_data, "chatbot": chatbot_data}
        return content
    except Exception as e:
...
def register():
    data = request.get_json(); return jsonify({"status": "success", "message": "Đăng ký thành công!"})

@app.route('/ask-goreal', methods=['POST'])
def ask_goreal():
    user_question = request.get_json().get('question', '')
    if not user_question: return jsonify({"answer": "Vui lòng đặt một câu hỏi."})
    
    if not model: return jsonify({"answer": "Xin lỗi, chức năng chatbot chưa được cấu hình API Key."})
