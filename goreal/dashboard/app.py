...
from ..config.settings import SHEET_NAME, CREDENTIALS_FILE, PLAYERLOG_SHEET, CHALLENGES_SHEET
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        st.info("Multi-tab dashboard for managing player logs and challenges with proof submission system")
        ...
        st.write("**Player Activity Log:** View and edit player challenge submissions with proof text")
...
def player_activity_tab():
    ...
    if df is None:
        st.error("Failed to load player log data from Google Sheets. Please check your connection and credentials.")
        return
    ...
    if df.empty:
        st.info("No player activity data available yet. Waiting for data from Roblox game...")
    else:
        ...
        st.info("💡 You can edit the data below. Review SubmissionText to verify player proofs and update Status accordingly.")
...
def challenge_management_tab():
    ...
    if df is None:
        st.error("Failed to load challenges data from Google Sheets. Please check your connection and credentials.")
        return
    ...
    st.info("💡 You can add, edit, or delete challenges below. Use the 'Save Challenge List' button to apply all changes.")
...
def main():
    ...
    st.markdown("*Dynamic Challenge System with Proof Submission - Multi-Tab Interface*")
    st.markdown("---")
    
    # Check if credentials file exists
    if not os.path.exists(CREDENTIALS_FILE):
        st.error(f"⚠️ {CREDENTIALS_FILE} not found. Please upload your Google Service Account credentials.")
        st.stop()
