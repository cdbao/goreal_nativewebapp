...
def display_playerlog_statistics(df):
    ...
    with col4:
        completed_count = len(df[df['Status'] == 'Completed']) if 'Status' in df.columns else 0
...
def display_challenges_statistics(df):
    ...
    with col2:
        avg_reward = df['RewardPoints'].mean() if 'RewardPoints' in df.columns and not df.empty else 0
        st.metric("Avg Reward Points", f"{avg_reward:.0f}")
    
    with col3:
        max_reward = df['RewardPoints'].max() if 'RewardPoints' in df.columns and not df.empty else 0
        st.metric("Max Reward Points", max_reward)
    
    with col4:
        min_reward = df['RewardPoints'].min() if 'RewardPoints' in df.columns and not df.empty else 0
...
def display_submission_reviews(df):
    ...
    if not submitted_entries.empty:
        ...
        for idx, row in submitted_entries.iterrows():
            with st.expander(f"📝 {row['PlayerName']} - {row['ChallengeID']} ({row['Timestamp']})"):
                st.write("**Submission:**")
...
def apply_filters(df, filters):
    ...
    if filters.get('challenges'):
        filtered_df = filtered_df[filtered_df['ChallengeID'].isin(filters['challenges'])]
