...
class GoogleSheetsClient:
    ...
    def connect(self) -> Tuple[Optional[Any], Optional[Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error connecting to Google Sheets: {str(e)}", exc_info=True)
            return None, None
    ...
    def get_worksheet(self, worksheet_name: str):
        ...
        try:
            ...
            self.logger.error(f"Error accessing worksheet '{worksheet_name}': {str(e)}", exc_info=True)
            return None
    
    def log_challenge(self, player_id: str, player_name: str, challenge_id: str, 
                     status: str = "Received", submission_text: str = "") -> bool:
        """
        ...
        try:
            ...
            row_data = [timestamp, str(player_id), str(player_name), 
                       str(challenge_id), status, submission_text]
            ...
            self.logger.error(f"Error logging challenge for player {player_id}, challenge {challenge_id}: {str(e)}", exc_info=True)
            return False
    
    def update_submission(self, player_id: str, challenge_id: str, 
                         submission_text: str, status: str = "Submitted") -> bool:
        """
        ...
        try:
            ...
            for idx in range(len(records) - 1, -1, -1):
                record = records[idx]
                if (str(record.get('PlayerID', '')) == str(player_id) and 
                    str(record.get('ChallengeID', '')) == str(challenge_id)):
            ...
            self.logger.error(f"Error updating submission for player {player_id}, challenge {challenge_id}: {str(e)}", exc_info=True)
            return False
    
    def get_player_status(self, player_id: str, challenge_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(f"Error getting player status for player {player_id}, challenge {challenge_id}: {str(e)}", exc_info=True)
            return None
