...
const AscensionPath: React.FC = () => {
  ...
  const [hoveredReward, setHoveredReward] = useState<{
    levelId: number;
    rewardIndex: number;
  } | null>(null);
  ...
  const levels: Level[] = [
    {
      ...
      requiredAura: 0,
      description:
        'Bước đầu tiên của hành trình. Mọi Vệ Thần đều bắt đầu từ đây.',
      rewards: [
        {
          icon: '🎯',
          name: 'Nhiệm Vụ Đầu Tiên',
          description: 'Quyền truy cập vào hệ thống nhiệm vụ cơ bản',
        },
        {
          icon: '📊',
          name: 'Theo Dõi Tiến Trình',
          description: 'Có thể xem thống kê AURA và streak cá nhân',
        },
      ],
      isUnlocked: true,
      isCurrent: false,
    },
    {
      ...
      requiredAura: 100,
      description:
        'Học hỏi những kiến thức nền tảng và rèn luyện kỷ luật bản thân.',
      rewards: [
        {
          icon: '🔥',
          name: 'Lửa Ý Chí',
          description: 'Tăng 10% tốc độ tích lũy AURA từ nhiệm vụ hàng ngày',
        },
        {
          icon: '📚',
          name: 'Sách Hướng Dẫn',
          description: 'Mở khóa thêm nhiều loại nhiệm vụ đa dạng',
        },
      ],
      isUnlocked: false,
      isCurrent: false,
    },
    {
      ...
      requiredAura: 500,
      description:
        'Bảo vệ bản thân khỏi sự cám dỗ và xây dựng thói quen bền vững.',
      rewards: [
        {
          icon: '🛡️',
          name: 'Giáp Tay Bền Bỉ',
          description:
            'Giảm 15% penalty khi miss streak, dễ dàng phục hồi động lực',
        },
        {
          icon: '⚡',
          name: 'Năng Lượng Tập Trung',
          description:
            'Bonus AURA khi hoàn thành liên tiếp nhiệm vụ trong ngày',
        },
      ],
      isUnlocked: false,
      isCurrent: false,
    },
    {
      ...
      requiredAura: 1500,
      description:
        'Trở thành tấm gương cho những người mới và chia sẻ kinh nghiệm.',
      rewards: [
        {
          icon: '👥',
          name: 'Quyền Cố Vấn',
          description: 'Có thể hướng dẫn và hỗ trợ Môn Sinh mới',
        },
        {
          icon: '🏆',
          name: 'Danh Hiệu Chiến Binh',
          description: 'Hiển thị danh hiệu đặc biệt trên profile',
        },
      ],
      isUnlocked: false,
      isCurrent: false,
      ...
      rewards: [
        {
          icon: '🌟',
          name: 'Aura Chân Nhân',
          description: 'AURA decay chậm hơn 50% khi không hoạt động',
        },
        {
          icon: '🎭',
          name: 'Quyền Tùy Chỉnh',
          description: 'Tùy chỉnh giao diện và tạo nhiệm vụ cá nhân',
        },
      ],
      isUnlocked: false,
      isCurrent: false,
    },
    {
      ...
      requiredAura: 10000,
      description:
        "Những huyền thoại sống, nguồn cảm hứng cho toàn bộ Titans' Guild.",
      rewards: [
        {
          icon: '🌈',
          name: 'Hào Quang Huyền Thoại',
          description: 'Aura trail đặc biệt hiển thị sau tên',
        },
        {
          icon: '🔮',
          name: 'Quyền Tối Thượng',
          description:
            'Truy cập vào các tính năng độc quyền và sự kiện đặc biệt',
        },
      ],
      isUnlocked: false,
      isCurrent: false,
  ...
  const getProgressToNextLevel = (
    ...
    const progress =
      ((aura - currentLevel.requiredAura) /
        (nextLevel.requiredAura - currentLevel.requiredAura)) *
      100;
        ...
        <div className="current-status">
          <div className="current-level">
            <span className="current-icon">
              {processedLevels[currentLevelIndex]?.icon}
            </span>
            <div className="current-info">
              <h3>{processedLevels[currentLevelIndex]?.name}</h3>
              <p>
                AURA hiện tại: <span className="aura-count">{currentAura}</span>
              </p>
            </div>
          </div>
          ...
          {currentLevelIndex < levels.length - 1 && (
            <div className="next-level-progress">
              ...
              <div className="aura-needed">
                Cần thêm:{' '}
                <span>
                  {levels[currentLevelIndex + 1].requiredAura - currentAura}{' '}
                  AURA
                </span>
              </div>
            </div>
        ...
        <div className="road-path">
          {processedLevels.map((level, index) => (
            <div key={level.id} className="road-segment">
              ...
              {index > 0 && (
                <div
                  className={`connection-line ${level.isUnlocked ? 'unlocked' : 'locked'}`}
                >
                ...
                <div className="level-info">
                  <div className="level-header">
                    ...
                    <div className="aura-requirement">
                      {level.requiredAura > 0
                        ? `${level.requiredAura} AURA`
                        : 'Điểm bắt đầu'}
                    </div>
                  </div>
                    ...
                    <div className="rewards-list">
                      {level.rewards.map((reward, rewardIndex) => (
                        <div
                          ...
                          onMouseEnter={() =>
                            setHoveredReward({ levelId: level.id, rewardIndex })
                          }
