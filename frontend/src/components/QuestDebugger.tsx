    ...
    try {
      ...
      try {
        ...
        info.tests.readQuests = {
          ...
          quests: questsSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          })),
        };
      ...
      try {
        const activeQuestsQuery = collection(
          db,
        ...
        info.tests.readActiveQuests = {
          ...
          activeQuests: activeQuestsSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          })),
        };
      ...
      try {
        const testQuestRef = doc(
          db,
          `users/${currentUser.uid}/activeQuests/test-quest-${Date.now()}`
        );
        ...
        <div>
          ...
          >
            ...
            <div>
              ...
              {debugInfo.user.displayName || 'N/A'}
            </div>
            <div>
              <strong>Guild:</strong> {debugInfo.user.guild || 'N/A'}
            </div>
            <div>
          ...
          {Object.entries(debugInfo.tests).map(
