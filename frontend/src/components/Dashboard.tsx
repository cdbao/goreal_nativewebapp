...
const Dashboard: React.FC = () => {
  ...
  const { displayedAura, updateAura } = useAura(userData?.currentAura || 0, currentUser?.uid);
  const { showCeremony, ceremonyData, closeCeremony } = useNotifications(currentUser?.uid);
  ...
  return (
    <div 
      ...
      style={{
        ...containerStyle,
        minHeight: '100vh',
        background: containerStyle.backgroundImage ? undefined : 'linear-gradient(135deg, #0a0e17 0%, #1a1a2e 30%, #16213e 70%, #0f1419 100%)',
        backgroundAttachment: 'fixed',
        color: 'white'
      }}
            ...
            {userData?.guild && (() => {
              ...
              return (
                <>
                  ...
                  <p className="text-elegant text-shadow">{guildInfo.description}</p>
                </>
              );
            })()}
          </div>
            ...
            <>
              <div className="player-info animate-fadeInUp">
                <div className="player-card frosted-glass">
                  ...
                  <div className="player-details">
                    <h2 className="text-fantasy text-primary">{userData?.displayName}</h2>
                    <div className="player-stats">
                      <div className="stat-item">
                        <span className="stat-label text-muted">Cấp độ:</span>
                        <span className="stat-value text-primary">{userData?.level}</span>
                      </div>
                      <div className="stat-item">
                        <span className="stat-label text-muted">AURA:</span>
                        <span className="stat-value aura text-secondary animate-glow">⚡{displayedAura}</span>
                      </div>
                      <div className="stat-item">
                        <span className="stat-label text-muted">Streak:</span>
                        <span className="stat-value streak text-primary">🔥{userData?.currentStreak}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="guild-master-message animate-fadeInUp">
                <div className="message-card frosted-glass">
                  {(() => {
                    const { master, message } = getGuildMasterMessage(userData?.guild, userData?.displayName);
                    return (
                      <>
                        <div className="guild-master-avatar animate-pulse">{master.avatar}</div>
                        <div className="message-content">
                          <h3 className="text-fantasy text-primary">{master.name}</h3>
