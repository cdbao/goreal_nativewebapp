...
const EnhancedQuestManager: React.FC<EnhancedQuestManagerProps> = ({ onAuraUpdate }) => {
  ...
  const loadQuestsData = async () => {
    ...
    try {
      ...
      for (const activeDoc of activeSnapshot.docs) {
        ...
        const acceptedDate = new Date(activeData.acceptedAt?.toDate?.() || activeData.acceptedAt);
  ...
  const setupActiveQuestsListener = () => {
    ...
    const activeQuestsRef = collection(db, `users/${currentUser.uid}/activeQuests`);
    return onSnapshot(activeQuestsRef, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        // Find the corresponding quest data
        const questData = availableQuests.find(q => q.questId === activeData.questId);
        if (questData) {
  ...
  const handleSubmissionComplete = async (submission: Submission) => {
    ...
    if (currentUser) {
      try {
        const activeQuestRef = doc(db, `users/${currentUser.uid}/activeQuests/${submission.questId}`);
    ...
    <div className="enhanced-quest-manager">
        ...
        {availableQuests.length === 0 ? (
          <div style={{ 
            ...
            <p style={{ fontSize: '1rem' }}>
              Hãy chờ {guildInfo.name === 'titans' ? 'Thủ lĩnh Kael' : 
                       guildInfo.name === 'illumination' ? 'Hiền triết Lyra' : 
                       'Sứ giả Zephyr'} công bố nhiệm vụ mới!
            </p>
          </div>
        ) : (
      ...
      {showCeremony && currentSubmission && (
        <AuraOfferingCeremony
          quest={availableQuests.find(q => q.questId === currentSubmission.questId)!}
